    ...
    <div>
        ...
        <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-5">
            <div v-for="item in stats" :key="item.name"
                :class="['overflow-hidden rounded-lg bg-white px-4 py-5 shadow sm:p-6 border border-b-4', 'border-b-' + item.color]">
                <dt class="truncate text-sm font-medium text-gray-500">{{ item.name }}</dt>
                <dd class="mt-1 text-3xl font-semibold tracking-tight text-gray-900">{{ item.stat }}</dd>
            </div>
        </dl>
        <div class="flex justify-center items-center w-full h-80 px-5 mt-5 bg-gray-300">
            <p>Line chart here!</p>
        </div>
    </div>
</template>
...
const stats = [
    { name: 'Fire Reports', stat: '5', color: 'primaryRed' },
    { name: 'Earthquake Reports', stat: '0', color: 'primaryYellow' },
    { name: 'Medical Reports', stat: '3', color: 'green-500' },
    { name: 'Flood Reports', stat: '1', color: 'blue-600' },
    { name: 'Police Reports', stat: '10', color: 'sky-400' },
]
</script>